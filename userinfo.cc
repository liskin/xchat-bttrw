...
retry1:
	...
	if (!s.getline(l))
	    return out;
	out.nick = nick;
	out.name = recode_to_client(l);
	
	if (!s.getline(l))
	    throw runtime_error("User info is not complete.");
	out.surname = recode_to_client(l);
	...
	if (!s.getline(l))
	    throw runtime_error("User info is not complete.");
	out.email = recode_to_client(l);

	if (!s.getline(l))
	    throw runtime_error("User info is not complete.");
	out.nick_created = recode_to_client(l);
	...
	if (!s.getline(l))
	    throw runtime_error("User info is not complete.");
	out.last_online = recode_to_client(l);

	if (!s.getline(l))
	    throw runtime_error("User info is not complete.");
	out.top_pos = recode_to_client(l);
